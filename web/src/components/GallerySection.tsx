  ...
  return (
    <section id="gallery" className="border-b border-stone-200 bg-[#f7f7f5] py-16 sm:py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="grid min-h-[260px] gap-4 md:grid-cols-3">
          {GALLERY_IMAGES.map((img, i) => (
            <motion.figure
              key={img.src}
              className="group relative min-h-[260px] overflow-hidden rounded-3xl border border-stone-200 bg-stone-200 shadow-sm"
              initial={{ opacity: 0, y: 16 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: 0.1 * (i + 1), duration: 0.45 }}
            >
              <button type="button" onClick={() => open(i)} className="relative block h-full w-full cursor-zoom-in">
                <Image
                  src={img.src}
                  alt={img.alt}
                  fill
